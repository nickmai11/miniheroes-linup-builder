...
Usage:  python3 scripts/slice-divinities.py            (needs Pillow + numpy)
        python3 scripts/slice-divinities.py --catalog-only  (metadata edits only)

After replacing icons, bump ASSET_VERSION in src/lib/asset-version.ts so browsers and
...
`kind` is the display category: the popup title without " Divinity", unless the
owner specifies a different grouping.
...
CATALOG = [
    ...
    (S("10.41.20"), "SPD Reduction RES", "Knockback"),
    (S("10.42.08"), "Heavy Injury", "Healing"),
    (S("10.41.26"), "Healing Effect", "Healing"),
    (S("10.41.40"), "Receive Healing", "Healing"),
...
def write_catalog(rows):
    """Write catalog metadata without changing the screenshot-derived icons."""
    with open(TS, "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--catalog-only", action="store_true", help="Regenerate seed data and SQL without slicing icons")
    args = parser.parse_args()
    if args.catalog_only:
        write_catalog([dict(slug=slugify(name), name=name, kind=kind) for _, name, kind in CATALOG])
        print(f"Wrote catalog metadata for {len(CATALOG)} divinities")
    else:
        main()
