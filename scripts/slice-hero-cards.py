...
If re-slicing changes an existing image, bump ASSET_VERSION in
src/lib/asset-version.ts so browsers and next/image drop their cached copies.
"""
import re, sys
import numpy as np
...
# purple shield=support). Rarity = Archive section / card colour
# (cyan-to-purple=eternal, red=mythic, gold=legend, purple=epic).
LAYOUT = [
    ("archive-mythic-warriors.png", ["Monkey King","Baphomet","White Ox","Swordevil","Wine Immortal","Dark Knight","Lucifer","Silver Warrior","Sea Captain"], [W]*9, "mythic"),
    ...
    ("image copy 9.png", ["Hellscream","Dark Queen"], [W,M], "eternal"),
    ("Dreamstar Spirit/Profile.png", ["Dreamstar Spirit"], [U], "mythic"),
    ("Nether Soul/Profile.png", ["Nether Soul"], [W], "mythic"),
    ("Nightmare Source/Profile.png", ["Nightmare Source"], [U], "mythic"),
    ("Panda Warrior/Profile.png", ["Panda Warrior"], [W], "mythic"),
]


# Verified overview, awaiting the owner's whole Archive card; use no substitute art.
PENDING_PORTRAITS = {}

# Small Archive cards need a precise diamond-only patch so nearby feet/armor
# remain intact. Coordinates are relative to the trimmed colored art panel.
GEM_PATCHES = {
    "nether-soul": {
        "box": (0, 149, 36, 185),
        "source": (0, 0, 36, 36),
        "diamond": [(18, 0), (35, 18), (18, 35), (0, 18)],
    },
}
...
def remove_artifact_gem(panel, patch=None):
    """Paint over the artifact-progress diamond (bottom-left) with a mirrored patch."""
    if patch:
        out = panel.copy()
        replacement = panel.crop(patch["source"]).transpose(Image.Transpose.FLIP_TOP_BOTTOM)
        mask = Image.new("L", replacement.size, 0)
        ImageDraw.Draw(mask).polygon(patch["diamond"], fill=255)
        mask = mask.filter(ImageFilter.GaussianBlur(0.6))
        out.paste(replacement, patch["box"][:2], mask)
        return out
...
def write_seed(heroes):
    ...
    rank = {"eternal": 0, "mythic": 1, "legend": 2, "epic": 3}
    lines = [
        ...
        "/**", " * Roster from owner screenshots (gameplay/heroes); portraits use Archive cards.",
        " * Generated by scripts/slice-hero-cards.py — edit LAYOUT/PENDING_PORTRAITS there.",
        " * Role/rarity use Archive cards or a verified overview pending its portrait",
        " * (cyan-to-purple = eternal, red = mythic, gold = legend, purple = epic).", " */",
    ...
    for role in order:
        ...
        for slug, name, r, rar in sorted((h for h in heroes if h[2] == role), key=lambda h: (rank[h[3]], h[1])):
            image = "null" if slug in PENDING_PORTRAITS else f'img("{slug}")'
            lines.append(f'  {{ slug: "{slug}", name: "{name}", role: "{r}", rarity: "{rar}", imageUrl: {image} }},')
        lines.append("")
    lines.append("];")
...
def write_upsert_sql(heroes):
    def image_value(slug):
        return "NULL" if slug in PENDING_PORTRAITS else f"'/heroes/{slug}.png'"

    values = ",\n".join(
        f"('{s}','{n.replace(chr(39), chr(39)*2)}','{r}','{ra}',{image_value(s)})" for s, n, r, ra in heroes
    )
...
def main():
    ...
    for file, names, roles, rarity in LAYOUT:
        ...
        for box, name, role in zip(boxes, names, roles):
            slug = slugify(name)
            portrait = remove_artifact_gem(art_panel(im.crop(box), rarity), GEM_PATCHES.get(slug))
            portrait.save(f"{OUT}/{slug}.png", optimize=True)
            heroes.append((slug, name, role, rarity))
    for slug, (name, role, rarity, source) in PENDING_PORTRAITS.items():
        with Image.open(f"{SRC}/{source}") as overview:
            overview.verify()
        heroes.append((slug, name, role, rarity))
    ...
    print(f"{len(heroes) - len(PENDING_PORTRAITS)} portraits, {len(PENDING_PORTRAITS)} pending -> {OUT}/, src/data/heroes.ts, scripts/upsert-heroes.sql")
