...
export function BuildPicker({
  ...
  return (
    <div className="flex min-w-0 flex-col gap-1.5">
      <span className="text-muted-foreground text-xs font-medium">Build</span>
      <Select.Root
        value={selectedId}
      ...
      >
        <div className="bg-background flex h-10 items-center rounded-md border">
          ...
          <Select.Trigger
            aria-label={`Choose build for ${heroName}`}
          ...
          >
            <Select.Value
              ...
              placeholder={builds.length ? "Select build…" : "No saved builds"}
            />
            <Select.Icon>
        ...
        <Select.Portal>
          ...
          >
            <Select.Popup
              aria-label={`Builds for ${heroName}`}
            ...
            >
              ...
              >
                ...
                <Select.ItemText>No build</Select.ItemText>
              </Select.Item>
              {builds.map((build) => (
                <BuildPopover
                  key={build.id}
