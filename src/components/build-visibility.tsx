...
import { setBuildVisibility } from "@/app/heroes/[slug]/build-actions";
...
export function BuildVisibilityToggle({
  isPrivate,
  onChange,
  disabled = false,
}: {
  ...
  return (
    <Tooltip>
      <TooltipTrigger
        ...
        aria-label={t(isPrivate ? "Show build" : "Hide build")}
        disabled={disabled}
      ...
      <BuildVisibilityToggle
        ...
        onChange={(next) => {
          ...
          startTransition(async () => {
            setSelected(next);
            try {
              const result = await setBuildVisibility(id, next);
